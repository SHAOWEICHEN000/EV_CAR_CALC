...
PV=setupsolar(2,0.22,0.95,700);                            %area, panel efficiency,DCDC efficiency,sun power
%----------- 1. 載入行車循跡 (可批次多條) -------------------------------
...
for idx = 1:numel(cycles)
    ...
    for k = 2:n
        %----- 2‑1 計算需求加速度/扭矩 ------------------------------
       %--------- 由油門 α 計算可用馬達扭矩 -----------------
        Vin     = alpha(k) / 100 * battery.vnom;          % 逆變器輸出電壓
        motRPM(k)= v(k-1)/wheel.radius * driveTr.gearRatio * 60/(2*pi);

        if motRPM(k) < motor.RPMrated
            TmaxAvail = motor.Lmax * Vin / battery.vnom;  % 線性降額
        else
            TmaxAvail = motor.Lmax * motor.RPMrated / motRPM(k) * Vin / battery.vnom;
        end
        ...
        %----- 2‑4 電池功率/電流/SOC -----------------------------
        Pmot  = (motRPM(k)+motRPM(k-1))/2 * 2*pi/60 * Tm(k) / (1000);  % kW
        %----- 2‑3.5  太陽能輸入功率 ------------------------- ?
        G = PV.G*ones(n,1);
        Ppv = G(k)*PV.area*PV.panel_eff*PV.DCDC_eff;   % kW

        if Pmot >= 0
            Pbat(k)= vehicle.overheadPwr+ Pmot/driveTr.efficiency-Ppv;%Accerlate
        else
            Pbat(k)= vehicle.overheadPwr+ Pmot*driveTr.efficiency-Ppv;%declate
        end
...
ylabel('Speed (m/s)')              % 需要 km/h 就 /3.6 後改單位
